...
namespace { // anon
template <class TExp, typename... Ts> expr_ptr make_expr(Ts&&... ts)
{
...
template <class Functor> struct unary final : expr, Functor {
  unary(expr_ptr p) : op1(std::move(p)) {}
  value_type operator()(value_type n) const override
  {
    return Functor::operator()(op1(n));
  }
...
#define BINOP_CASE(match, cls)                                                 \
  case match:                                                                  \
    return make_expr<cls>(std::move(left), std::move(right))
  ...
  default:
    throw std::logic_error("Unexpected binary operator"); // LCOV_EXCL_LINE
  }
#undef BINOP_CASE
}
...
class tokenizer {
public:
  tokenizer(const char* s) : text_(s), next_tocken_(0), numeric_value_(0)
  {
    step();
  }
...
private:
  ...
  void step()
  {
    ...
    if (is(text, "&&")) {
      ...
      numeric_value_ = std::min<unsigned long long>(
          std::numeric_limits<long long>::max(), value);
      text_ = tmp_ptr;
      next_tocken_ = NUM;
    }
    else if (*text == '\0')
  ...
  expr_ptr unary_expr()
  {
    ...
    if (is_in(t.next(), level_unary)) {
      ...
      if (!op1)
        return expr_ptr();
      if (BOOST_LIKELY(op == '!'))
        return make_expr<unary<std::logical_not<expr::value_type>>>(
            std::move(op1));
      else {
        BOOST_ASSERT(op == '-');
...
#undef BINARY_EXPR
  ...
  expr_ptr cond_expr()
  {
    ...
    if (!(case2 = cond_expr()))
      return expr_ptr();
    return make_expr<conditional>(std::move(cond), std::move(case1),
                                  std::move(case2));
  }
